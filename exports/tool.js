// This function will hold our entire application
function mainApp() {
    /**
    ...
    const ANV = {
        ...
        util: {
            ...
            exportToCsv: (filename, rows) => {
                ...
                ANV.util.downloadBlob(csvContent, filename, 'text/csv;charset=utf-8;');
            },
            downloadJson: (data, filename) => {
                const jsonContent = JSON.stringify(data, null, 2);
                ANV.util.downloadBlob(jsonContent, filename, 'application/json');
            },
        },
                ...
                const renderSimplePreview = () => {
                    ...
                    <div class="invoice-preview-wrapper">
                        ...
                        <div class="invoice-print-preview">
                            <h3>Print Preview</h3>
                            ${generateSimplePrintHTML('cafe', 'preview')}
                        </div>
                    </div>`;
                };

                const generateSimplePrintHTML = (type, mode = 'print') => {
                    ...
                    const logoSetting = (settings.logoUrl || '').trim();
                    const qrSetting = (settings.qrCodeUrl || '').trim();
                    const logoUrl = logoSetting.toLowerCase() === 'none' ? '' : resolveAssetUrl(logoSetting, './logo.png');
                    const qrCodeUrl = qrSetting.toLowerCase() === 'none' ? '' : resolveAssetUrl(qrSetting, './upi-qr-code.png');
                    const showQr = qrCodeUrl !== '';
                    const formattedAddress = (settings.address || '').replace(/\n/g, '<br>');
                    const formattedNotes = notes.replace(/\n/g, '<br>');
                    const wrapperClass = mode === 'preview' ? 'isp-wrapper isp-preview' : 'isp-wrapper isp-print';
                    let totalsHtml;
                    if (isVoucher) {
                    ...
                    const itemsHtml = items.length > 0 ? items.map(item => `<tr><td>${item.item || item.name}</td><td>${billDate}</td><td>${item.qty}</td><td>${ANV.util.money(item.rate)}</td><td>${ANV.util.money(item.amount)}</td></tr>`).join('') : `<tr><td colspan="5" style="text-align:center;">No items added.</td></tr>`;
                    const qrBlock = showQr ? `<div class="isp-header-right"><img src="${qrCodeUrl}" class="isp-qr-code" alt="UPI QR Code"><p>${settings.name || ''}<br>${settings.paymentRecipientName || ''}</p></div>` : '<div class="isp-header-right"></div>';
                    return `
                    <div class="${wrapperClass}">
                        <div class="isp-header">
                            <div class="isp-header-left">
                                ${logoUrl ? `<img src="${logoUrl}" class="isp-logo" alt="${settings.name || 'Brand Logo'}">` : ''}
                        ...
                        <div class="isp-footer"><p>${formattedNotes}</p></div>
                    </div>`;
                };
        ...
        handlers: {
            ...
            onAddCafeItem: () => {
                ...
                if (!menuItem || qty <= 0) { console.error("Could not find menu item with ID:", menuItemId); return; }
            ...
            onPrint: (type) => {
                ...
                printArea.innerHTML = type === 'cafe' ? ANV.elements.invoice.printArea.dataset.cafeBillHtml : ANV.elements.invoice.printArea.dataset.voucherHtml;
            ...
            onSaveBooking: (e) => {
                ...
                const cafeBill = ANV.activeBookingId ? ANV.util.num(ANV.state.bookings.find(b=>b.id===ANV.activeBookingId).cafeBill) : 0;
                const updatedBooking = { guest: bookingData.guest, phone: bookingData.phone, email: bookingData.email, source: bookingData.source, room: bookingData.room, checkIn: bookingData.checkIn, checkOut: bookingData.checkOut, nights: nights, roomRate: roomRate, stayPaid: form.elements.stayPaid.checked, cafePaid: form.elements.cafePaid.checked, notes: bookingData.notes, cafeBill: cafeBill, cafeItems: ANV.activeBookingId ? ANV.state.bookings.find(b=>b.id===ANV.activeBookingId).cafeItems : [], };
                ...
                if (ANV.activeBookingId) {
                    ...
                    updatedBooking.id = ANV.util.uid('BK'); updatedBooking.invoiceNo = ANV.util.nextInvoiceNo(); ANV.state.bookings.push(updatedBooking);
                }
                ANV.stateManager.save(); ANV.render.all(); ANV.hideModal();
            },
            onDeleteBooking: (bookingId) => {
                if (confirm('Are you sure you want to delete this booking? This cannot be undone.')) {
                    ANV.state.bookings = ANV.state.bookings.filter(b => b.id !== bookingId); ANV.stateManager.save(); ANV.render.all();
                }
            },
            ...
            onSaveExpense: (e) => {
                ...
                if (ANV.activeExpenseId) {
                    ...
                    updatedExpense.id = ANV.activeExpenseId; ANV.state.expenses[index] = updatedExpense;
                } else {
                    updatedExpense.id = ANV.util.uid('EX'); ANV.state.expenses.push(updatedExpense);
                }
                ANV.stateManager.save(); ANV.render.all(); ANV.hideModal();
            },
            onDeleteExpense: (expenseId) => {
                if (confirm('Are you sure you want to delete this expense? This cannot be undone.')) {
                    ANV.state.expenses = ANV.state.expenses.filter(e => e.id !== expenseId); ANV.stateManager.save(); ANV.render.all();
                }
            },
        ...
        initCharts: () => {
            ...
            const chartContexts = { cafeVsStay: document.getElementById('cafeVsStayChart'), revenueBySource: document.getElementById('revenueBySourceChart'), expenseBreakdown: document.getElementById('expenseBreakdownChart') };
    ...
    // Expose the namespace globally for inline handlers and debugging
    window.ANV = ANV;

    // This robustly waits for both the DOM and the config file to be ready
    document.addEventListener('DOMContentLoaded', ANV.init);

}

// This wrapper ensures that the config file is loaded before the main app runs
if (window.ANV_CONFIG) {
    mainApp();
} else {
    window.addEventListener('configLoaded', mainApp);
}
